  ...
  //your code here
    ...
    for(int i = 0;i < msg->mytbl->tbl.size(); i++ )
    {
        ...
        if(check==0)
        {
            ...
            {
                new_entry.cost = min(16, msg->mytbl->tbl[i].share_cost + 1);
            }
            else
            {
            ...
            new_entry.share_cost = new_entry.cost;
            //insert in table
        ...
        else
        {
            ...
            {
                mytbl.tbl[ind].cost = min(16, msg->mytbl->tbl[i].share_cost + 1);
                mytbl.tbl[ind].nexthop = msg->from;
            }
            else if(mytbl.tbl[ind].cost > msg->mytbl->tbl[i].share_cost + 1)
            {
                mytbl.tbl[ind].cost = min(16, msg->mytbl->tbl[i].share_cost + 1);
                mytbl.tbl[ind].nexthop = msg->from;
            }
            mytbl.tbl[ind].share_cost = mytbl.tbl[ind].cost;
        }
