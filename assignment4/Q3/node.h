...
class Node {
 ...
 public:
  ...
  void updateTblEntry()
  {
      //if B is asking for update table entry then change the cost of entry where destip is C to 16
      if(getName() == "B")
      {
          for(int i = 0; i < mytbl.tbl.size(); i++ )
          {
            if(mytbl.tbl[i].dstip == "10.0.1.3"){
              mytbl.tbl[i].cost = 16;
            } 
          }
      }
      //if C is asking for update table entry then change the cost of entry where destip is B to 16
      else
      {
          for(int i = 0; i < mytbl.tbl.size(); i++ )
          {
            if(mytbl.tbl[i].dstip == "10.0.1.23"){
              mytbl.tbl[i].cost = 16;
            } 
          }
      }
  ...
  void sendMsg(){
    ...
    for (int i = 0; i < interfaces.size(); ++i) {
      ...
      //if nexthop and recv ip are same then pass the share cost as 16(infinite) else pass it same as it was
      for (int j = 0; j < msg.mytbl->tbl.size(); ++j)
      {
        /* code */
        if(msg.mytbl->tbl[j].nexthop == msg.recvip)
        {
          msg.mytbl->tbl[j].share_cost = 16;
        }
        else
        {
          msg.mytbl->tbl[j].share_cost = msg.mytbl->tbl[j].cost;
        }
      }		
